# Projet V — site vitrine

Base frontend moderne pour un site vitrine rapide, responsive et optimisé pour
le SEO local, l’AEO (Answer Engine Optimization) et le GEO (Generative Engine
Optimization).
...
- Tailwind CSS 4
- composants UI accessibles Shadcn
- hébergement Sites

Le projet reste volontairement **frontend only** : aucune base de données,
aucune authentification et aucune API métier ne sont utilisées.

## Architecture prévue

- `app/` : pages, layouts, métadonnées et styles globaux
- `components/` : composants réutilisables et primitives UI
- `config/` : informations de marque, coordonnées et zone locale
- `lib/seo/` : données structurées et helpers de référencement
- `public/` : images et fichiers statiques optimisés

Les informations réelles de l’entreprise, la ville ciblée, les services et le
nom de domaine seront ajoutés avant d’activer le sitemap, les données structurées
LocalBusiness et les pages locales.
